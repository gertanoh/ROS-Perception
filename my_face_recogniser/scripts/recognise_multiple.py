...
class FaceRecogniser(object):
    ...
    def recognise(self,data):
        ...
        image_path_person = os.path.join(self.path_to_package,"person_img/standing_person.png")
        standing_person_image = face_recognition.load_image_file(image_path_person)
        standing_person_face_encoding = face_recognition.face_encodings(standing_person_image)[0]
        
        
        image_path_naoko = os.path.join(self.path_to_package,"person_img/naoko.png")
        naoko_image = face_recognition.load_image_file(image_path_naoko)
        naoko_face_encoding = face_recognition.face_encodings(naoko_image)[0]
        
        
        image_path_bobeye = os.path.join(self.path_to_package,"person_img/bobeye.png")
        bob_image = face_recognition.load_image_file(image_path_bobeye)
        ...
        if process_this_frame:
            ...
            for face_encoding in face_encodings:
                ...
                if face_recognition.compare_faces([standing_person_face_encoding], face_encoding)[0]:
                    rospy.loginfo("MATCH")
                    name = "StandingPerson"
                
                elif face_recognition.compare_faces([naoko_face_encoding], face_encoding)[0]:
                    name = " Naoko"
                    
                elif face_recognition.compare_faces([bob_face_encoding], face_encoding)[0]:
                    name = " BobEye"
